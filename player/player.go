// Package player package to store player information
package player
...
func (p *PPlayer) SetLastFlashedBy(attackerID int64) { p.lastFlashedBy = attackerID }
...
// GetPistolRoundLost get number of bomb has been planted
...
// GetSide get side of player
func (p *PPlayer) GetSide() (string, bool) {

	if p == nil {
		log.WithFields(log.Fields{
			"name": p.Name,
			"id":   p.UserID,
		}).Error("The player has no side: ")
		return "", false
	}

	switch p.Team {
	case player.TeamTerrorists:
		return "T", true
	case player.TeamCounterTerrorists:
		return "CT", true
	}
...
func (p *PPlayer) NotifySpecialRoundWon(RoundType string) {
	switch RoundType {
	case "pistol_round":
		p.pistolRoundWon++
	case "eco_round":
		p.ecoRoundsWon++
	case "force_buy_round":
		p.forceRoundsWon++
	default:
...
func (p *PPlayer) NotifySpecialRoundLost(RoundType string) {
	switch RoundType {
	case "pistol_round":
		p.pistolRoundslost++
	case "eco_round":
...
// // IsBlinded check player is currently blinded
// func (p *PPlayer) IsBlinded() bool {
